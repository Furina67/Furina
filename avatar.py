...
class AvatarView(discord.ui.View):
    ...
    def __init__(self, user: discord.User):
        ...
        super().__init__(timeout=60)  # buttons work for 1 minute
    ...
    async def update_embed(self, interaction: discord.Interaction, show_avatar: bool):
        ...
        # If user clicks the same button again

        if show_avatar == self.showing_avatar:

            await interaction.response.send_message(

                f"⚠️ You're already viewing the {'avatar' if show_avatar else 'banner'}!",
    ...
    async def avatar_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await self.update_embed(interaction, show_avatar=True)
    ...
    async def banner_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await self.update_embed(interaction, show_avatar=False)
...
class Avatar(commands.Cog):
    ...
    async def avatar(self, ctx, user: discord.User = None):
        ...
        """Show a user's avatar and banner with buttons (no ping)."""

        user = user or ctx.author

        user = await self.bot.fetch_user(user.id)  # fetch banner info
        ...
        await ctx.send(embed=embed, view=view)  # 👈 no reply, no ping
